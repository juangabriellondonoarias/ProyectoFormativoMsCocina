...
public class RecetaMapper {
    ...
    public static Receta toEntity(RecetaDTO dto) {
        ...
        if (dto.getIngrediente() != null) {
            ...
            for (RecetaIngredienteDTO ingDTO : dto.getIngrediente()) {
                ...
                // Configurar clave compuesta y relación padre
                ri.setReceta(receta); // VITAL: Vincular con el padre
                
                // Referencia al ingrediente de stock
    ...
    public static RecetaDTO toDTO(Receta entity) {
        ...
        if (entity.getPasos() != null) {
            List<PasoPreparacionDTO> pasosDTO = entity.getPasos().stream()
        ...
        if (entity.getIngredientes() != null) {
            List<RecetaIngredienteDTO> ingredientesDTO = entity.getIngredientes().stream()
                    .map(ri -> {
                        RecetaIngredienteDTO ingDTO = new RecetaIngredienteDTO();
                        ingDTO.setIdIngrediente(
                            ri.getIngrediente() != null 
                                ? ri.getIngrediente().getIdIngrediente()
                                : null
                        );
